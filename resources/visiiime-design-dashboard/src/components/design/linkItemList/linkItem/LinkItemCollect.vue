    ...
    <v-ob ref="vob" tag="div" class="p-2">
        ...
        <div class="flex justify-center align-middle">
            ...
            <v-p v-slot="{ errors }" rules="required" name="表單標題" tag="div" class="w-full">
                <input v-model="linkItem.collector.collectTitle" :disabled="linkItem.online" type="text" class="w-full text-xs p-2 focus:border-red-100 outline-none" autocomplete="off" placeholder="表單標題" @blur="validate" required />
                <div class="mb-5 w-full text-red-500 text-xs">{{ errors[0] }}</div>
            </v-p>
        </div>
        <div class="flex justify-center align-middle mb-2">
            ...
            <v-p v-slot="{ errors }" rules="required" name="送出回應訊息" tag="div" class="w-full">
                <input v-model="linkItem.collector.collectRsp" :disabled="linkItem.online" type="text" class="w-full text-xs p-2 focus:border-red-100 outline-none" autocomplete="off" placeholder="送出回應訊息" @blur="validate" required />
                <div class="mb-5 w-full text-red-500 text-xs">{{ errors[0] }}</div>
            </v-p>
        </div>
    </v-ob>
...
import { CAN_USE_LINK_ITEM_COLLECTOR } from "@/enum/permission/vBasic/VPermission";
...
export default {
    ...
    methods: {
        async validate() {
            const rs = await this.$store.getters.hasPermission(CAN_USE_LINK_ITEM_COLLECTOR);
            if (!rs) {
                // todo ...
