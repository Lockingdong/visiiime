<template>
    <div class="flex mb-5">
        <div class="h-6 w-6 text-gray-700 handle">
            <vs-icon :icon-name="`${socialLink.fa}-${socialLink.icon}`"/>
        </div>
        <div class="ml-3 border-gray-500 w-full text-sm">
            <v-ob ref="vob">
                <v-p v-slot="{ errors, classes }" :rules="rulesAndName.rules" :name="rulesAndName.fieldName" tag="div" class="w-full">
                    <input
                        v-show="editable"
        ...
        <div>
            <svg @click="openEdit" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600 fill-current cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
            </svg>
        </div>
        <div>
            <svg @click="removeSocialLink" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-300 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        rulesAndName() {
            ...
            switch (this.socialLink.icon) {
                case "envelope":
                    ...
                    break;
                default:
                    rule = `url|${this.socialLink.icon}Social`;
                    fieldName = "連結";
                    break;
            }
