<template>
  <div class="flex">
    <button
      class="w-full py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-500 hover:bg-indigo-400 focus:outline-none"
      @click="addLinkItem(linkTypeEnum.normal)"
    >ADD LINK</button>
    <button
      @click="featureButtonOn = !featureButtonOn"
      class="relative ml-2 w-30 py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-500 hover:bg-indigo-400 focus:outline-none"
    >
      <fai :icon="['fas', 'star']" size="lg" />
      <div v-show="featureButtonOn" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1" class="
          z-10
          origin-top-right
          absolute
          right-0
...
export default {
  ...
  methods: {
    addLinkItem(linkType) {
      this.$emit('add-link-item', linkType);
    }
  }
};
</script>
